export default {
  ...
  keys: {
    ...
    multiGroupRoutingHint: '开启后可为该密钥配置多个分组并按优先级路由（高优先级分组优先，同级按会话分配并保持会话稳定）。',
    multiGroupRoutingEditorHint: '优先级数字越小越优先。高优先级分组不可用时自动降级到下一优先级；同一优先级内相同会话会尽量保持在同一个分组。',
    multiGroupRequired: '请至少配置一个多分组路由条目',
    priority: '优先级',
    removeGroup: '移除该分组',
    addGroup: '添加分组',
    usage: '用量',
