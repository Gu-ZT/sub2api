export default {
  ...
  keys: {
    ...
    multiGroupRoutingHint: 'Configure multiple groups with priorities for this key (higher priority first; same priority distributes by session and stays stable per session).',
    multiGroupRoutingEditorHint: 'Smaller priority number means higher priority. When the high-priority group is unavailable it falls back to the next priority; within the same priority the same session stays in the same group.',
    multiGroupRequired: 'Please configure at least one multi-group route entry',
    priority: 'Priority',
